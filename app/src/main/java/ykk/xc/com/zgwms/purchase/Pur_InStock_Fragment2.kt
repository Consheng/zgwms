...
import ykk.xc.com.zgwms.basics.AssistantData_CarVersion_DialogActivity
import ykk.xc.com.zgwms.basics.MoreBatchInputDialog
import ykk.xc.com.zgwms.basics.Mtl_DialogActivity
        ...
        override fun handleMessage(msg: Message) {
            ...
            if (m != null) {
                ...
                when (msg.what) {
                    ...
                    SAVE -> { // 保存成功 进入
                        // 保存了分录，供应商就不能修改
                        m.parent!!.fragment1.poOrderEntryList = null
                        m.parent!!.fragment1.purReceiveEntryList = null
...
//                                }
    ...
    fun showLocalStockGroup(key :String) {
        ...
        // 显示调入仓库---------------------
        if(spfStock.contains(key+"_BIND_PUR_STOCK")) {
            stock = showObjectByXml(Stock_K3::class.java, key+"_BIND_PUR_STOCK", saveDefaultStock)
        ...
        /*if(spfStock.contains(STOCKPOS_FLAG)) {
            stockPos = showObjectByXml(StockPosition_K3::class.java, STOCKPOS_FLAG, saveOther)
    ...
    override fun setUserVisibleHint(isVisibleToUser: Boolean) {
        ...
        if (isVisibleToUser) {
            if(parent!!.fragment1.icstockBill.fselTranType == 0) {
                btn_mtlSel.visibility = View.VISIBLE
            } else {
                btn_mtlSel.visibility = View.GONE
            }
            if(parent!!.fragment1.poOrderEntryList != null) {
                // 执行保存功能
                setICStockEntry_POOrder(parent!!.fragment1.poOrderEntryList)

            } else if(parent!!.fragment1.purReceiveEntryList != null) {
                // 执行保存功能
                setICStockEntry_POInStock(parent!!.fragment1.purReceiveEntryList)
            }
            mHandler.sendEmptyMessageDelayed(SETFOCUS, 200)
        }
    }
    ...
    fun onViewClicked(view: View) {
        when (view.id) {
            ...
            R.id.btn_mtlSel -> { // 选择物料
                smqFlag = '2'
                val bundle = Bundle()
                bundle.putInt("fuseOrgId", parent!!.fragment1.icstockBill.fstockOrgId)
...
//        icstockBillEntry.fdcSPId = 0
    ...
    fun getICStockBillEntry(icEntry: ICStockBillEntry) {
        ...
        /*if(icEntry.material.personal > 0) {
            lin_carSeries.visibility = View.VISIBLE
            lin_carVersion.visibility = View.VISIBLE
            tv_carSeriesSel.text = icEntry.carSeriesName
            tv_carTypeSel.text = icEntry.carTypeName
            tv_carVersionSel.text = icEntry.carVersionName
        } else {
            lin_carSeries.visibility = View.GONE
            lin_carVersion.visibility = View.GONE
            tv_carSeriesSel.text = ""
            tv_carTypeSel.text = ""
            tv_carVersionSel.text = ""
        }*/
...
//            entry.fentryId = it.fentryid
            entry.fdcStockId = if(stock != null) stock!!.fstockId else 0
            entry.fdcSPId = if(stockPos != null) stockPos!!.fid else 0
//            entry.fqty = it.useableQty
            ...
            entry.fsourceBillNo = it.poOrder.fbillNo
            entry.forderInterId = it.fid
            entry.forderEntryId = it.fentryId
            entry.forderSeq = it.fseq
            entry.forderBillNo = it.poOrder.fbillNo
//            entry.fsourceQty = it.fqty
            entry.fsourceQty = it.usableQty
            entry.fsrcBillTypeId = "PUR_PurchaseOrder"
            entry.fentity_Link_FRuleId = "PUR_PurchaseOrder-STK_InStock"
            entry.fentity_Link_FSTableName = "t_PUR_POOrderEntry"
            entry.fownerNumber = it.requireOrg.fnumber
            entry.fownerName = it.requireOrg.fname
    ...
    private fun setICStockEntry_POInStock(list : List<PurReceiveEntry>?) {
        var listEntry = ArrayList<ICStockBillEntry>()
        list!!.forEach {
            val entry = ICStockBillEntry()
            entry.icstockBillId = parent!!.fragment1.icstockBill.id
...
//            entry.fqty = it.useableQty
            ...
            entry.fsourceInterId = it.fid
            entry.fsourceEntryId = it.fentryId
            entry.fsourceBillNo = it.purReceive.fbillNo
            entry.forderInterId = it.fid
            entry.forderEntryId = it.fentryId
            entry.forderBillNo = it.purReceive.fbillNo
//            entry.fsourceQty = it.fqty
            entry.fsourceQty = it.usableQty
            entry.fsrcBillTypeId = "PUR_ReceiveBill"
            entry.fentity_Link_FRuleId = "PUR_ReceiveBill-STK_InStock"
            entry.fentity_Link_FSTableName = "T_PUR_ReceiveEntry"
            entry.fownerNumber = it.purReceive.needOrg.fnumber
            entry.fownerName = it.purReceive.needOrg.fname
    ...
    private fun run_smDatas(mtlId :Int) {
        ...
        var icstockBillId = ""
        var billType = ""
        var searchBillType = if(parent!!.fragment1.icstockBill.fselTranType == 0) "0" else "1" // 查询单据类型：0：无源单，1：有源单
        when(smqFlag) {
            '1' -> {

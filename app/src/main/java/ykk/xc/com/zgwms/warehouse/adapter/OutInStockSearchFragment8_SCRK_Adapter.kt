...
class OutInStockSearchFragment8_SCRK_Adapter(private val context: Activity, datas: List<ICStockBill>) : BaseArrayRecyclerAdapter<ICStockBill>(datas) {
    ...
    override fun onBindHoder(holder: BaseRecyclerAdapter.RecyclerHolder, entity: ICStockBill, pos: Int) {
        ...
        if(entity.stockOrg != null) {
            tv_recOrgName.text = Html.fromHtml("入库组织:&nbsp;<font color='#6a5acd'>"+ entity.stockOrg.fname +"</font>")
        } else {
            tv_recOrgName.text = "入库组织:"
        }
        if(entity.stockOutOrg != null) {
            tv_purOrgName.text = Html.fromHtml("生产组织:&nbsp;<font color='#6a5acd'>"+ entity.stockOutOrg.fname +"</font>")
        } else {
            tv_purOrgName.text = "生产组织:"
        }
        if(entity.recDept != null) {
            tv_suppName.text = Html.fromHtml("车间:&nbsp;<font color='#FF4400'>"+ entity.recDept.fname +"</font>")
        } else {
            tv_suppName.text = "车间:"
        }
        tv_empName.visibility = View.GONE   // 业务员不显示

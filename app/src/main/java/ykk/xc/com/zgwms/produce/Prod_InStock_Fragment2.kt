...
//                                }
    ...
    fun showLocalStockGroup(key :String) {
        ...
        // 显示调入仓库---------------------
        if(spfStock.contains(key+"_BIND_PROD_STOCK")) {
            stock = showObjectByXml(Stock_K3::class.java, key+"_BIND_PROD_STOCK", saveDefaultStock)
    ...
    override fun setUserVisibleHint(isVisibleToUser: Boolean) {
        ...
        if (isVisibleToUser) {
            ...
            if(parent!!.fragment1.prdMoEntryList != null) {
                // 执行保存功能
                setICStockEntry_PrdMo(parent!!.fragment1.prdMoEntryList)
            }
            mHandler.sendEmptyMessageDelayed(SETFOCUS, 200)
        }
    }
    ...
    fun onViewClicked(view: View) {
        when (view.id) {
            ...
            R.id.btn_mtlSel -> { // 选择物料
                ...
                bundle.putString("fiscoby", "1") // 可为联副产品
...
//                        }
    ...
    private fun setICStockEntry_PrdMo(list : List<PrdMoEntry>?) {
...
//            entry.fqty = it.useableQty
            ...
            entry.fsourceBillNo = it.prdMo.fbillNo
            entry.fsourceSeq = it.fseq
            entry.forderInterId = it.fsaleOrderId
            entry.forderEntryId = it.fsaleOrderEntryId
            entry.forderBillNo = it.fsaleOrderNo
            entry.forderSeq = it.fsaleOrderEntrySeq

...
class OutInStockSearchFragment6_FBSDRD_Adapter(private val context: Activity, datas: List<ICStockBill>) : BaseArrayRecyclerAdapter<ICStockBill>(datas) {
    ...
    override fun onBindHoder(holder: BaseRecyclerAdapter.RecyclerHolder, entity: ICStockBill, pos: Int) {
        ...
        val tv_stockOutOrgName = holder.obtainView<TextView>(R.id.tv_stockOutOrgName)
        ...
        if(entity.stockOutOrg != null) {
            tv_stockOutOrgName.text = Html.fromHtml("调出库存组织:&nbsp;<font color='#6a5acd'>"+ entity.stockOutOrg.fname +"</font>")
        } else {
            tv_stockOutOrgName.text = "调出库存组织:"
        }
        if(entity.stockOrg != null) {
            tv_stockOrgName.text = Html.fromHtml("调入库存组织:&nbsp;<font color='#6a5acd'>"+ entity.stockOrg.fname +"</font>")
        } else {
            tv_stockOrgName.text = "调入库存组织:"
        }

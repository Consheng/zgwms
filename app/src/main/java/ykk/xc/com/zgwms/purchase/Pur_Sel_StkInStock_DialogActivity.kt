...
import ykk.xc.com.zgwms.purchase.adapter.Pur_Sel_StkInStock_DialogAdapter
 ...
 * 选择采购入库单单号dialog
 */
class Pur_Sel_StkInStock_DialogActivity : BaseDialogActivity(), XRecyclerView.LoadingListener {
    ...
    private val listDatas = ArrayList<StkInStock>()
    private var mAdapter: Pur_Sel_StkInStock_DialogAdapter? = null
    private val okHttpClient = OkHttpClient()
    private var limit = 1
    private var isRefresh = false
    private var isLoadMore = false
    private var isNextPage = false
    private var fstockOrgId = 0   // 使用组织
    private var fpurChaseOrgId = 0   // 使用组织
    private var fsupplierId = 0 // 供应商id
    private var isTextChange: Boolean = false // 是否进入TextChange事件
    private var strFid :String? = null
    private var curSupplierId = 0 // 当前选择的供应商id
    private var returnValidData = 0 // 显示可以退货的入库单据分录

    // 消息处理
    private val mHandler = MyHandler(this)

    private class MyHandler(activity: Pur_Sel_StkInStock_DialogActivity) : Handler() {
        private val mActivity: WeakReference<Pur_Sel_StkInStock_DialogActivity>
        ...
        override fun handleMessage(msg: Message) {
            ...
            if (m != null) {
                ...
                when (msg.what) {
                    ...
                    SUCC2 -> { // 成功
//                        val list = JsonUtil.strToList2(msg.obj as String, StkInStock::class.java)
                        val list = JsonUtil.strToList(msgObj, StkInStock::class.java)
...
//                        m.xRecyclerView.isLoadingMoreEnabled = m.isNextPage
    ...
    override fun setLayoutResID(): Int {
        return R.layout.pur_sel_stkinstock_list_dialog
    }
    ...
    override fun initView() {
        ...
        mAdapter = Pur_Sel_StkInStock_DialogAdapter(context, listDatas)
    ...
    override fun initData() {
        ...
        if (bundle != null) {
            ...
            fpurChaseOrgId = bundle.getInt("fpurChaseOrgId")
            fsupplierId = bundle.getInt("fsupplierId")
            returnValidData = bundle.getInt("returnValidData",0)
        }
    ...
    private fun run_smDatas() {
        ...
        var mUrl = getURL("stkInStock/findBarcodeGetFid")
    ...
    private fun run_okhttpDatas() {
        showLoadDialog("加载中...", false)
        val mUrl = getURL("stkInStock/findPortionByParam_Parent")
        val formBody = FormBody.Builder()
                .add("fstockOrgId", fstockOrgId.toString())
                .add("fpurChaseOrgId",fpurChaseOrgId.toString())
                .add("fsupplierId",fsupplierId.toString())
                .add("strSourceNo", isNULLS(strFid))
                .add("returnValidData", returnValidData.toString()) // 显示可以退货的入库单据分录

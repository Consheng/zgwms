 ...
 * 作者：ykk
 */
class Other_OutStock_Fragment1 : BaseFragment() {
    ...
    companion object {
        ...
        private val SEL_PUR_DEPT = 65
        private val SEL_CUST = 66
    ...
    private var parent: Other_OutStock_MainActivity? = null
...
//    var isReset = false // 是否点击了重置按钮.
    ...
    private class MyHandler(activity: Other_OutStock_Fragment1) : Handler() {
        private val mActivity: WeakReference<Other_OutStock_Fragment1>
...
//                        m.run_purReceiveList()
    ...
    override fun initView() {
        ...
        parent = mContext as Other_OutStock_MainActivity
    }
...
//        hideSoftInputMode(mContext, et_code)
        ...
        icstockBill.billType = "QTCK" // 其他入库
        icstockBill.ftranType = 11
        ...
        icstockBill.fbillTypeNumber = "QTCKD01_SYS"
    ...
    @OnClick(R.id.tv_inDateSel, R.id.tv_fownerTypeSel, R.id.tv_fownerSel, R.id.tv_stockOrgSel, R.id.tv_stockOutOrgSel, R.id.tv_custSel,
...
//                bundle.putInt("fuseOrgId", icstockBill.fstockOutOrgId)
    ...
    fun checkSave(isHint :Boolean) : Boolean {
        ...
        if (icstockBill.fcustId == 0 && icstockBill.purDeptId == 0) {
            if(isHint) Comm.showWarnDialog(mContext, "客户与部门必须选择一项！")
            return false
        }
        return true
    }
    ...
    fun reset() {
        icstockBill.billType = "QTCK" // 无源单入库
        icstockBill.fselTranType = 0
        icstockBill.fstockDirect = "GENERAL" // 库存方向：（GENERAL:普通，RETURN:退货）

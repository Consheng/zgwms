 ...
 */
public class Comm {
	public static final String publicPaths = Environment.getExternalStorageDirectory().toString()+"/zgwms/";
...
//	}
	 ...
	 * 得到有效的手机号，去除无效的字符串
	 */
	public static String getValidMobile(String mobile) {
		if(isNULLS(mobile).length() == 0) return "";

		if(mobile.indexOf(" ") > -1) {
			mobile = mobile.replace(" ","");
//			mobile = mobile.replaceAll("\\s*", ""); // \s 匹配任何空白字符，包括空格、制表符、换页符等等。等价于 [ \f\n\r\t\v];
		}

		// ‭?15222221464‬? ,当为这种格式，就处理
		if(!Character.isDigit(mobile.charAt(0)) || !Character.isDigit(mobile.charAt(mobile.length()-1))) {
			mobile = mobile.substring(1, mobile.length()-1);
		}

        for(int i=0, len=mobile.length(); i<len; i++) {
			char mb = mobile.charAt(i);
			if (!Character.isDigit(mb) && mb != '-' && mb != '—') {

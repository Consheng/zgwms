...
import ykk.xc.com.zgwms.warehouse.adapter.OutInStockSearchFragment6_FBSDRD_Adapter
 ...
 * 作者：ykk
 */
class OutInStock_Search_Fragment6_FBSDRD : BaseFragment() {
    ...
    private var mAdapter: OutInStockSearchFragment6_FBSDRD_Adapter? = null
    ...
    private class MyHandler(activity: OutInStock_Search_Fragment6_FBSDRD) : Handler() {
        private val mActivity: WeakReference<OutInStock_Search_Fragment6_FBSDRD>
...
//                        }
    ...
    override fun initView() {
        ...
        mAdapter = OutInStockSearchFragment6_FBSDRD_Adapter(mContext!!, checkDatas)
        recyclerView.adapter = mAdapter
        // 设值listview空间失去焦点
        recyclerView.isFocusable = false

        // 行事件
        mAdapter!!.setCallBack(object : OutInStockSearchFragment6_FBSDRD_Adapter.MyCallBack {
            override fun onSearch(entity: ICStockBill, position: Int) {
                val bundle = Bundle()
                bundle.putInt("id", entity.id)
                show(Ware_TransferIn_MainActivity::class.java, bundle)
            }
...
//        run_findList()
    ...
    private fun run_findList() {
        ...
        val formBody = FormBody.Builder()
                ...
                .add("strBillType", "FBSDRD") // 根据不同类型查询

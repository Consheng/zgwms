...
class OutInStockSearchFragment10_SalOutStock_Adapter(private val context: Activity, datas: List<ICStockBill>) : BaseArrayRecyclerAdapter<ICStockBill>(datas) {
    ...
    override fun onBindHoder(holder: BaseRecyclerAdapter.RecyclerHolder, entity: ICStockBill, pos: Int) {
        ...
        if(entity.strSourceNo != null && entity.strSourceNo.length > 0) {
            tv_strSourceNo.visibility = View.VISIBLE
            tv_strSourceNo.text = Html.fromHtml("订单号:&nbsp;<font color='#6a5acd'>"+entity.strSourceNo+"</font>")
        } else {
            tv_strSourceNo.visibility = View.INVISIBLE
        }
        if(entity.stockOrg != null) {
            tv_recOrgName.text = Html.fromHtml("发货组织:&nbsp;<font color='#6a5acd'>"+ entity.stockOrg.fname +"</font>")
        } else {
            tv_recOrgName.text = "发货组织:"
        }
        if(entity.stockOutOrg != null) {
            tv_purOrgName.text = Html.fromHtml("销售组织:&nbsp;<font color='#6a5acd'>"+ entity.stockOutOrg.fname +"</font>")
        } else {
            tv_purOrgName.text = "销售组织:"
        }
        if(entity.cust != null) {
        ...
        tv_empName.text = Html.fromHtml("销售员:&nbsp;<font color='#000000'>"+ entity.empName +"</font>")

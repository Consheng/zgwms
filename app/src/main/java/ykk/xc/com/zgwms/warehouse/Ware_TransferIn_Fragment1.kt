...
import kotlinx.android.synthetic.main.ware_transfer_in_fragment1.*
...
import ykk.xc.com.zgwms.bean.warehouse.StkTransferOutEntry
 ...
 * 作者：ykk
 */
class Ware_TransferIn_Fragment1 : BaseFragment() {
    ...
    companion object {
        ...
        private val SEL_TRANSFEROUT = 68
    ...
    private var parent: Ware_TransferIn_MainActivity? = null
    private val df = DecimalFormat("#.###")
    private var timesTamp:String? = null // 时间戳
    var icstockBill = ICStockBill() // 保存的对象
//    var isReset = false // 是否点击了重置按钮.
var stkTransferOutEntryList:List<StkTransferOutEntry>? = null
    ...
    private class MyHandler(activity: Ware_TransferIn_Fragment1) : Handler() {
        private val mActivity: WeakReference<Ware_TransferIn_Fragment1>
        ...
        override fun handleMessage(msg: Message) {
            ...
            if (m != null) {
                ...
                when (msg.what) {
                    ...
                    FIND_SOURCE_PUR ->{ // 查询分步式调出单 返回
                        val list = JsonUtil.strToList(msgObj, StkTransferOutEntry::class.java)
                        m.getStkTransferOutData(list)
                    }
                    UNFIND_SOURCE_PUR ->{ // 查询分步式调出单 失败！ 返回
                        errMsg = JsonUtil.strToString(msgObj)
...
//                        m.run_purReceiveList()
    ...
    fun setICStockBill(m : ICStockBill) {
        ...
        tv_sourceOrderSel.text = m.strSourceNo
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.ware_transfer_in_fragment1, container, false)
    }

    override fun initView() {
        mContext = getActivity()
        parent = mContext as Ware_TransferIn_MainActivity
    }
...
//        hideSoftInputMode(mContext, et_code)
        ...
        icstockBill.billType = "FBSDRD" // 分步式调入单
        icstockBill.ftranType = 42
...
//        icstockBill.empName = user!!.staff.fname
        ...
        icstockBill.fbillTypeNumber = "FBDR01_SYS"
    ...
    @OnClick(R.id.tv_inDateSel, R.id.tv_sourceOrderSel, R.id.tv_fownerTypeSel, R.id.tv_fownerOutTypeSel, R.id.tv_fownerSel, R.id.tv_fownerOutSel,
             R.id.tv_stockOrgSel, R.id.tv_stockOutOrgSel, R.id.tv_emp1Sel, R.id.tv_emp2Sel, R.id.btn_save, R.id.btn_clone)
    fun onViewClicked(view: View) {
        ...
        when (view.id) {
            ...
            R.id.tv_sourceOrderSel -> {
                bundle = Bundle()
                bundle.putInt("fstockOutOrgId" , icstockBill.fstockOutOrgId) // 调出库存组织
                bundle.putInt("fstockInOrgId" , icstockBill.fstockOrgId) // 调入库存组织
                bundle.putString("ftransferBizType", icstockBill.ftransferType) // 调拨类型
                bundle.putString("fobjectTypeId", "STK_TRANSFEROUT") // 业务类型
                bundle.putString("fownerTypeInId", icstockBill.fownerType) // 调入货主类型
                bundle.putString("fownerTypeOutId", icstockBill.fownerOutType) // 调出货主类型
                showForResult(Ware_Sel_TransferOut_DialogActivity::class.java, SEL_TRANSFEROUT, bundle)
            }
            R.id.tv_fownerOutTypeSel -> { // 调出货主类型选择
                pop_fownerType2(view)
...
//                bundle.putInt("fuseOrgId", icstockBill.fstockOutOrgId)
    ...
    override fun setListener() {
        ...
        rb_transferType2.setOnCheckedChangeListener { buttonView, isChecked ->
            if(isChecked) {
                ...
                lin_fownerOutType.visibility = View.VISIBLE
                lin_fownerOut.visibility = View.VISIBLE
                // 调拨类型：（InnerOrgTransfer:组织内调拨，OverOrgTransfer:跨组织调拨）
                ...
                tv_fownerOutSel.text = icstockBill.stockOutOrg.fname
            }
        }
    }
    ...
    fun checkSave(isHint :Boolean) : Boolean {
        ...
        if(getValues(tv_sourceOrderSel).length == 0 || stkTransferOutEntryList == null) {
            if(isHint) Comm.showWarnDialog(mContext, "请选择分步式调出单！")
            return false;
        }
        // 调拨类型：（InnerOrgTransfer:组织内调拨，OverOrgTransfer:跨组织调拨）
    ...
    fun reset() {
        ...
        lin_fownerOutType.visibility = View.GONE
        lin_fownerOut.visibility = View.GONE
...
//                    showForResult(Organization_DialogActivity::class.java, SEL_OWNER_ORG, null)
    ...
    /**
     * 创建PopupWindow 【调出货主类型 】
     */
    private var popWindow2: PopupWindow? = null
    private fun pop_fownerType2(v: View) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            when (requestCode) {
                ...
                SEL_OUT_ORG -> { // 调出库存组织
                    ...
                    icstockBill.stockOutOrg = org
                    tv_stockOutOrgSel.text = org.fname
                    // 如果调出货主类型为业务组织，调出货主就为调入组织
                    if(icstockBill.fownerOutType.equals("BD_OwnerOrg")) {
                        icstockBill.fownerOutNumber = org.fnumber
                        icstockBill.fownerOutName = org.fname
                        tv_fownerOutSel.text = org.fname
                    }
...
//                    }
                ...
                SEL_TRANSFEROUT -> { // 查询分步式调出单	返回
                    val transferOut = data!!.getSerializableExtra("obj") as StkTransferOut
                    run_transferOutList(transferOut.fid.toString())
                }
                SEL_EMP1 -> {//查询领料人	返回
...
//        if(checkSave(false)) run_save()
     ...
     * 分步式调出单返回
     */
    private fun getStkTransferOutData(list :List<StkTransferOutEntry>) {
        icstockBill.fselTranType = 1    // 分步式调出单
        stkTransferOutEntryList = list
        val stkTransferOut = list[0].stkTransferOut

        icstockBill.fstockOutOrgId = stkTransferOut.fstockOrgId
        icstockBill.fstockOrgId = stkTransferOut.fstockInOrgId
        icstockBill.stockOutOrg = stkTransferOut.stockOutOrg
        icstockBill.stockOrg = stkTransferOut.stockInOrg
        icstockBill.fbillTypeNumber = "FBDR01_SYS"
        icstockBill.setFbusinessTypeNumber("FBDC01_SYS")

        tv_sourceOrderSel.text = stkTransferOut.fbillNo
        // 禁止修改项
        if(rb_transferType1.isChecked) {
            rb_transferType2.isEnabled = false
        }
        if(rb_transferType2.isChecked) {
            rb_transferType1.isEnabled = false
        }
        setEnables(tv_stockOrgSel, R.drawable.back_style_gray2a, false)
        setEnables(tv_stockOutOrgSel, R.drawable.back_style_gray2a, false)
...
//                        if(checkSave(false)) run_save()
     ...
     * 查询分步式调出单
     */
    private fun run_transferOutList(finterid :String) {
        isTextChange = false
        showLoadDialog("保存中...", false)
        val mUrl = getURL("stkTransferOut/findEntryByParam")

...
import kotlinx.android.synthetic.main.ware_transfer_in_fragment2.*
import okhttp3.*
 ...
 * 作者：ykk
 */
class Ware_TransferIn_Fragment2 : BaseFragment() {
    ...
    private class MyHandler(activity: Ware_TransferIn_Fragment2) : Handler() {
        private val mActivity: WeakReference<Ware_TransferIn_Fragment2>
        ...
        override fun handleMessage(msg: Message) {
            ...
            if (m != null) {
                ...
                when (msg.what) {
                    ...
                    SAVE -> { // 保存成功 进入
                        // 保存了分录，供应商就不能修改
                        m.parent!!.fragment1.stkTransferOutEntryList = null
...
//                        m.toasts("保存成功✔")
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.ware_transfer_in_fragment2, container, false)
    }
    ...
    override fun setUserVisibleHint(isVisibleToUser: Boolean) {
        ...
        if (isVisibleToUser) {
            ...
            if(parent!!.fragment1.stkTransferOutEntryList != null) {
                // 执行保存功能
                setICStockEntry_TransferOut(parent!!.fragment1.stkTransferOutEntryList)
            }
...
//                        }
    ...
    private fun setICStockEntry_TransferOut(list : List<StkTransferOutEntry>?) {
...
//            entry.fentryId = it.fentryid
            entry.fdcStockId = it.fdestStockId
            entry.fdcSPId = it.fdestStockLocId
            entry.fscStockId = it.fsrcStockId
            entry.fscSPId = it.fsrcStockLocId
//            entry.fqty = it.useableQty
            entry.fprice = 0.0
            entry.funitId = it.funitId
            entry.fsourceInterId = it.fid
            entry.fsourceEntryId = it.fentryId
            entry.fsourceBillNo = it.stkTransferOut.fbillNo
            entry.forderInterId = it.fid
            entry.forderEntryId = it.fentryId
            entry.forderBillNo = it.stkTransferOut.fbillNo
//            entry.fsourceQty = it.fqty
            entry.fsourceQty = it.usableQty
            entry.fsrcBillTypeId = "STK_TRANSFEROUT"
            entry.fentity_Link_FRuleId = "STK_TRANSFEROUT-STK_TRANSFERIN"
            entry.fentity_Link_FSTableName = "T_STK_STKTRANSFEROUTENTRY"

...
import kotlinx.android.synthetic.main.ware_transfer_out_fragment1.*
 ...
 * 作者：ykk
 */
class Ware_TransferOut_Fragment1 : BaseFragment() {
...
//    var isReset = false // 是否点击了重置按钮.
    ...
    private class MyHandler(activity: Ware_TransferOut_Fragment1) : Handler() {
        private val mActivity: WeakReference<Ware_TransferOut_Fragment1>
...
//                        m.run_purReceiveList()
    ...
    fun setICStockBill(m : ICStockBill) {
        ...
        icstockBill.fvestonWay = m.fvestonWay
        ...
        // 分步式调出单--在途归属：（A:调出方，B:调入方）
        if(m.fvestonWay == 'A') {
            rb_fvestonWay1.isChecked = true
            rb_fvestonWay2.isChecked = false
            rb_fvestonWay2.isEnabled = false
        } else {
            rb_fvestonWay2.isChecked = true
            rb_fvestonWay1.isChecked = false
            rb_fvestonWay1.isEnabled = false
        }
        if(m.stockOrg != null) {
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.ware_transfer_out_fragment1, container, false)
    }
...
//        hideSoftInputMode(mContext, et_code)
        ...
        icstockBill.billType = "FBSDCD" // 分步式调出单
        icstockBill.ftranType = 41
        ...
        icstockBill.fvestonWay = 'A' // 分步式调出单--在途归属：（A:调出方，B:调入方）
...
//        icstockBill.empName = user!!.staff.fname
        ...
        icstockBill.fbillTypeNumber = "FBDC01_SYS"
    ...
    @OnClick(R.id.tv_inDateSel, R.id.tv_fownerOutTypeSel, R.id.tv_fownerOutSel, R.id.tv_fownerSel, R.id.tv_stockOrgSel, R.id.tv_stockOutOrgSel,
...
//                bundle.putInt("fuseOrgId", icstockBill.fstockOutOrgId)
    ...
    override fun setListener() {
        ...
        // 调出方
        rb_fvestonWay1.setOnCheckedChangeListener { buttonView, isChecked ->
            if(isChecked) {
                icstockBill.fvestonWay = 'A'
            }
        }
        // 调入方
        rb_fvestonWay2.setOnCheckedChangeListener { buttonView, isChecked ->
            if(isChecked) {
                icstockBill.fvestonWay = 'B'
            }
        }
    }

...
class OutInStockSearchFragment3_QTCK_Adapter(private val context: Activity, datas: List<ICStockBill>) : BaseArrayRecyclerAdapter<ICStockBill>(datas) {
    ...
    override fun onBindHoder(holder: BaseRecyclerAdapter.RecyclerHolder, entity: ICStockBill, pos: Int) {
        ...
        if(entity.stockOutOrg != null) {
            tv_purOrgName.text = Html.fromHtml("领料组织:&nbsp;<font color='#6a5acd'>"+ entity.stockOutOrg.fname +"</font>")
        } else {
            tv_purOrgName.text = "领料组织:"
        }
        if(entity.cust != null) {
        ...
        tv_empName.text = Html.fromHtml("领料人:&nbsp;<font color='#000000'>"+ entity.empName +"</font>")

 ...
 * 作者：ykk
 */
class Sal_OutStock_Fragment2 : BaseFragment() {
    ...
    private class MyHandler(activity: Sal_OutStock_Fragment2) : Handler() {
        private val mActivity: WeakReference<Sal_OutStock_Fragment2>
        ...
        override fun handleMessage(msg: Message) {
            ...
            if (m != null) {
                ...
                when (msg.what) {
                    ...
                    SAVE -> { // 保存成功 进入
                        // 保存了分录，供应商就不能修改
                        m.parent!!.fragment1.salOrderEntryList = null
                        m.parent!!.fragment1.deliveryNoticeEntryList = null
...
//                                }
    ...
    fun showLocalStockGroup(key :String) {
        ...
        // 显示调入仓库---------------------
        if(spfStock.contains(key+"_BIND_SAL_OUT_STOCK")) {
            stock = showObjectByXml(Stock_K3::class.java, key+"_BIND_SAL_OUT_STOCK", saveDefaultStock)
    ...
    override fun setUserVisibleHint(isVisibleToUser: Boolean) {
        ...
        if (isVisibleToUser) {
            ...
            if(parent!!.fragment1.salOrderEntryList != null) {
                // 执行保存功能
                setICStockEntry_Order(parent!!.fragment1.salOrderEntryList)

            } else if(parent!!.fragment1.deliveryNoticeEntryList != null) {
                // 执行保存功能
                setICStockEntry_deliveryNotice(parent!!.fragment1.deliveryNoticeEntryList)
            }
            mHandler.sendEmptyMessageDelayed(SETFOCUS, 200)
        }
    }
...
//        }
        ...
        if(icstockBillEntry.material.personal > 0 && icstockBillEntry.carVersionNumber == null) {
            Comm.showWarnDialog(mContext, "请输入车型版号！")
            return false
        }
        return true;
    }
...
//                        }
    ...
    private fun setICStockEntry_Order(list : List<SalOrderEntry>?) {
...
//            entry.fsourceQty = it.fqty
    ...
    private fun setICStockEntry_deliveryNotice(list : List<DeliveryNoticeEntry>?) {
        var listEntry = ArrayList<ICStockBillEntry>()
        list!!.forEach {
            val entry = ICStockBillEntry()
            entry.icstockBillId = parent!!.fragment1.icstockBill.id
            entry.mtlId = it.fmaterialId
//            entry.fentryId = it.fentryid
            entry.fdcStockId = it.fshipmentStockId
            entry.fdcSPId = it.fshipmentStockLocId
//            entry.fqty = it.useableQty
            entry.fprice = it.fprice
            entry.funitId = it.funitId
            entry.fsourceInterId = it.fid
            entry.fsourceEntryId = it.fentryId
            entry.fsourceBillNo = it.deliveryNotice.fbillNo
            entry.forderInterId = it.fid
            entry.forderEntryId = it.fentryId
            entry.forderBillNo = it.deliveryNotice.fbillNo
//            entry.fsourceQty = it.fqty
            entry.fsourceQty = it.usableQty
            entry.fsrcBillTypeId = "SAL_DELIVERYNOTICE"
            entry.fentity_Link_FRuleId = "DeliveryNotice-OutStock"
            entry.fentity_Link_FSTableName = "T_SAL_DELIVERYNOTICEENTRY"
            if(it.material.personal > 0 && isNULLS(it.personalCarVersionNumber).length > 0) {
                entry.carVersionNumber = it.personalCarVersionNumber
//                entry.carVersionLocation = it.personalCarVersionLocation
                entry.carVersionName = it.personalCarVersionName

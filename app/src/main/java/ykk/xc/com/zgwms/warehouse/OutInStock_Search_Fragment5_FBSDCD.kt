...
import ykk.xc.com.zgwms.warehouse.adapter.OutInStockSearchFragment5_FBSDCD_Adapter
 ...
 * 作者：ykk
 */
class OutInStock_Search_Fragment5_FBSDCD : BaseFragment() {
    ...
    private var mAdapter: OutInStockSearchFragment5_FBSDCD_Adapter? = null
    ...
    private class MyHandler(activity: OutInStock_Search_Fragment5_FBSDCD) : Handler() {
        private val mActivity: WeakReference<OutInStock_Search_Fragment5_FBSDCD>
...
//                        }
    ...
    override fun initView() {
        ...
        mAdapter = OutInStockSearchFragment5_FBSDCD_Adapter(mContext!!, checkDatas)
        recyclerView.adapter = mAdapter
        // 设值listview空间失去焦点
        recyclerView.isFocusable = false

        // 行事件
        mAdapter!!.setCallBack(object : OutInStockSearchFragment5_FBSDCD_Adapter.MyCallBack {
            override fun onSearch(entity: ICStockBill, position: Int) {
                val bundle = Bundle()
                bundle.putInt("id", entity.id)
                show(Ware_TransferOut_MainActivity::class.java, bundle)
            }
...
//        run_findList()
    ...
    private fun run_findList() {
        ...
        val formBody = FormBody.Builder()
                ...
                .add("strBillType", "FBSDCD") // 根据不同类型查询

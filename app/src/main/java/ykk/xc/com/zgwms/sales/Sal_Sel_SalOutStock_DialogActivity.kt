...
import kotlinx.android.synthetic.main.sal_sel_saloutstock_list_dialog.*
import okhttp3.*
import ykk.xc.com.zgwms.R
import ykk.xc.com.zgwms.bean.warehouse.SalOutStock
...
import ykk.xc.com.zgwms.sales.adapter.Sal_Sel_SalOutStock_DialogAdapter
 ...
 * 选择销售出库单号dialog
 */
class Sal_Sel_SalOutStock_DialogActivity : BaseDialogActivity(), XRecyclerView.LoadingListener {
    ...
    private val listDatas = ArrayList<SalOutStock>()
    private var mAdapter: Sal_Sel_SalOutStock_DialogAdapter? = null
    private val okHttpClient = OkHttpClient()
    private var limit = 1
    private var isRefresh = false
    private var isLoadMore = false
    private var isNextPage = false
    private var fstockOrgId = 0   // 发货组织
    private var fsaleOrgId = 0   // 采购组织
    private var fcustomerId = 0 // 客户id
    private var isTextChange: Boolean = false // 是否进入TextChange事件
    private var strFid :String? = null
    private var curCustId = 0 // 当前选择的供应商id

    // 消息处理
    private val mHandler = MyHandler(this)

    private class MyHandler(activity: Sal_Sel_SalOutStock_DialogActivity) : Handler() {
        private val mActivity: WeakReference<Sal_Sel_SalOutStock_DialogActivity>
        ...
        override fun handleMessage(msg: Message) {
            ...
            if (m != null) {
                ...
                when (msg.what) {
                    ...
                    SUCC2 -> {// 查询列表   成功
                        val list = JsonUtil.strToList(msg.obj as String, SalOutStock::class.java)
...
//                        m.xRecyclerView.isLoadingMoreEnabled = m.isNextPage
    ...
    override fun setLayoutResID(): Int {
        return R.layout.sal_sel_saloutstock_list_dialog
    }
    ...
    override fun initView() {
        ...
        mAdapter = Sal_Sel_SalOutStock_DialogAdapter(context, listDatas)
        ...
        mAdapter!!.onItemClickListener = BaseRecyclerAdapter.OnItemClickListener { adapter, holder, view, pos ->
            val m = listDatas[pos - 1]
            if(curCustId > 0 && curCustId != m.fcustomerId) {
                Comm.showWarnDialog(context,"请选择相同供应商！")

            } else {
                curCustId = m.fcustomerId
                if (m.isCheck) {
                    m.isCheck = false
                } else {
                    m.isCheck = true
    ...
    override fun initData() {
        ...
        if (bundle != null) {
            fstockOrgId = bundle.getInt("fstockOrgId")
            fsaleOrgId = bundle.getInt("fsaleOrgId")
            fcustomerId = bundle.getInt("fcustomerId")
        }
    ...
    private fun run_smDatas() {
        ...
        var mUrl = getURL("salOutStock/findBarcodeGetFid")
...
//                .add("strFid", if(strFid != null) strFid else "")
    ...
    private fun run_okhttpDatas() {
        showLoadDialog("加载中...", false)
        val mUrl = getURL("salOutStock/findPortionByParam_Parent")
        val formBody = FormBody.Builder()
                .add("fstockOrgId", fstockOrgId.toString())
                .add("fsaleOrgId", fsaleOrgId.toString())
                .add("fcustomerId", fcustomerId.toString())

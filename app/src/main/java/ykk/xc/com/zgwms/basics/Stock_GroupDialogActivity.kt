...
class Stock_GroupDialogActivity : BaseDialogActivity() {
                ...
                when (msg.what) {
                    SUCC1 -> { // 扫码成功 进入
                        when(m.smqFlag) {
                            ...
                            '2' -> { // 库位扫码
                                val stockPos = JsonUtil.strToObject(msgObj, StockPosition_K3::class.java)
    ...
    override fun initData() {
        ...
        if (bundle != null) {
            ...
            if(bundle.containsKey("stock") && bundle.getSerializable("stock") != null) {
                stock = bundle.getSerializable("stock") as Stock_K3
                getStock(stock!!,false)
            }
            if(bundle.containsKey("stockPos") && bundle.getSerializable("stockPos") != null) {
                stockPos = bundle.getSerializable("stockPos") as StockPosition_K3
                getStockPos(stockPos!!,false)
            }
        }
        if(stock == null) {
            val bundle = Bundle()
            bundle.putInt("fuseOrgId", fuseOrgId)
    ...
    fun onViewClicked(view: View) {
        when (view.id) {
            ...
            R.id.btn_stockPosSel -> { // 库位
                val bundle = Bundle()
                bundle.putInt("fstockId", if(stock != null)stock!!.fstockId else 0)
                bundle.putInt("fuseOrgId", fuseOrgId)
            ...
            R.id.btn_confirm -> { // 确认
                ...
                if(stock!!.fisOpenLocation == 1 && stockPos == null) { // 是否启用了库位
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            when (requestCode) {
                SEL_STOCK -> {// 仓库	返回
                    stock = data!!.getSerializableExtra("obj") as Stock_K3
                    getStock(stock!!, true)
                }
                SEL_STOCKPOS -> { // 库位	返回
                    stockPos = data!!.getSerializableExtra("obj") as StockPosition_K3
                ...
                BaseFragment.CAMERA_SCAN -> {// 扫一扫成功  返回
                    ...
                    if (hmsScan != null) {
                        when (smqFlag) {
                            ...
                            '2' -> setTexts(et_stockPosCode, hmsScan.originalValue)
                        }
                    }
                }
                WRITE_CODE -> {
     ...
     * 得到仓库
     */
    fun getStock(stock :Stock_K3, autoNext :Boolean) {
        context.stock = stock
     ...
     * 得到库位
     */
    fun getStockPos(stockPos : StockPosition_K3, autoNext :Boolean) {
     ...
     */
    fun getStockGroup(stock: Stock_K3?, stockPos : StockPosition_K3?) {
    ...
    private fun run_smDatas() {
        ...
        when(smqFlag) {
            ...
            '2' -> {
                mUrl = getURL("StockPosition_K3/findBarcode")
                barcode = getValues(et_stockPosCode)
            }
        }
        val formBody = FormBody.Builder()
                .add("barcode", barcode)
                .add("stockId", if(stock != null) stock!!.fstockId.toString() else "" )
                .build()

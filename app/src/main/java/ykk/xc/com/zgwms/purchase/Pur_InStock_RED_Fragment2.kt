        ...
        override fun handleMessage(msg: Message) {
            ...
            if (m != null) {
                ...
                when (msg.what) {
                    ...
                    SAVE -> { // 保存成功 进入
                        // 保存了分录，供应商就不能修改
                        m.parent!!.fragment1.stkInStockEntryList = null
...
//                                }
    ...
    fun showLocalStockGroup(key :String) {
        ...
        // 显示调入仓库---------------------
        if(spfStock.contains(key+"_BIND_PUR_STOCK_RED")) {
            stock = showObjectByXml(Stock_K3::class.java, key+"_BIND_PUR_STOCK_RED", saveDefaultStock)
    ...
    override fun setUserVisibleHint(isVisibleToUser: Boolean) {
        ...
        if (isVisibleToUser) {
            ...
            if(parent!!.fragment1.stkInStockEntryList != null) {
                // 执行保存功能
                setICStockEntry_StkInStock(parent!!.fragment1.stkInStockEntryList)
            }
            mHandler.sendEmptyMessageDelayed(SETFOCUS, 200)
        }
    }
...
//        tv_unitSel.text = icItem.unit.unitName
        ...
        icstockBillEntry.mtlId = icItem.fitemid
        icstockBillEntry.funitId = icItem.unit.funitId
        // 满足条件就查询库存
        if(icItem.inventoryQty <= 0 && icstockBillEntry.fdcStockId > 0 && icstockBillEntry.mtlId > 0) {
            run_findInventoryQty()
        }
    }

    private fun setICStockEntry_StkInStock(list : List<StkInStockEntry>?) {
...
//            entry.fentryId = it.fentryid
            ...
            if(it.fstockId == 0 && stock != null) {
                entry.fdcStockId = stock!!.fstockId
            }
            if(it.fstockLocId == 0 && stockPos != null) {
                entry.fdcSPId = stockPos!!.fid
            }
//            entry.fqty = it.useableQty
            ...
            entry.fsourceBillNo = it.stkInStock.fbillNo
            entry.forderInterId = it.fid
            entry.forderEntryId = it.fentryId
            entry.forderBillNo = it.stkInStock.fbillNo
//            entry.fsourceQty = it.fqty
            entry.fsourceQty = it.usableQty
            entry.fsrcBillTypeId = "STK_InStock"
            entry.fentity_Link_FRuleId = "STK_InStock-PUR_MRB"
            entry.fentity_Link_FSTableName = "t_STK_InStockEntry"

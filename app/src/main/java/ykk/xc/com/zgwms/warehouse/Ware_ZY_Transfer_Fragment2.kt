...
import kotlinx.android.synthetic.main.ware_zy_transfer_fragment2.*
import okhttp3.*
 ...
 * 作者：ykk
 */
class Ware_ZY_Transfer_Fragment2 : BaseFragment() {
    ...
    private var parent: Ware_ZY_Transfer_MainActivity? = null
    ...
    private class MyHandler(activity: Ware_ZY_Transfer_Fragment2) : Handler() {
        private val mActivity: WeakReference<Ware_ZY_Transfer_Fragment2>
...
//                        m.toasts("保存成功✔")
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.ware_zy_transfer_fragment2, container, false)
    }

    override fun initView() {
        mContext = getActivity()
        parent = mContext as Ware_ZY_Transfer_MainActivity
...
//            cb_outRemember.isEnabled = false
            ...
            tv_positionName.isEnabled = false
...
//            cb_inRemember.isEnabled = false
    ...
    fun showLocalStockGroup(key :String) {
        ...
        // 显示调出仓库---------------------
        if(spfStock.contains(key+"_BIND_ZYDB_OUT_STOCK")) {
            stock2 = showObjectByXml(Stock_K3::class.java, key+"_BIND_ZYDB_OUT_STOCK", saveDefaultStock)
            tv_outPositionName.text = stock2!!.fname
        }
        // 显示调入仓库---------------------
        if(spfStock.contains(key+"_BIND_ZYDB_IN_STOCK")) {
            stock = showObjectByXml(Stock_K3::class.java, key+"_BIND_ZYDB_IN_STOCK", saveDefaultStock)
